...
DEFAULT_UTILS=("pre-commit" "flake9" "black" "isort" "mypy" "tox" "pdm" "virtualenv")
FLAKE8_UTILS=("pep8-naming" "flake8-isort" "flake8-black" "flake8-bandit" "flake8-django" "flake8-bugbear" "flake8-simplify" "flake8-pytest-style" "flake8-cognitive-complexity" "flake8-expression-complexity")
TOX_UTILS=("tox-pdm")
...
for flake8_util in ${FLAKE8_UTILS[@]}; do
    if ! type ${flake8_util} > /dev/null 2>&1; then
        ${pipx_path}pipx inject --system-site-packages --pip-args '--no-cache-dir --force-reinstall' flake9 ${flake8_util}
    else
        echo "${flake8_util} already installed. Skipping."
    fi
done
for tox_util in ${TOX_UTILS[@]}; do
    if ! type ${tox_util} > /dev/null 2>&1; then
        ${pipx_path}pipx inject --system-site-packages --pip-args '--no-cache-dir --force-reinstall' tox ${tox_util}
    else
        echo "${tox_util} already installed. Skipping."
    fi
done
rm -rf /tmp/pip-tmp
